...
class EEGSEPDataHandler(EmbeddingDataHandler):
    """Built in embedding data handler for EEGSEP system
    """
    class EEGSEPDataset(Dataset):
        """Dataset for training EEGSEP embedding model.
    ...
    @dataclass
    class EEGSEPDataCollator:
        """Data collator for EEGSEP embedding problem
        """
        # Default collator
        def __call__(self, examples:List[Dict[str, torch.Tensor]]) -> Dict[str, torch.Tensor]:
            # Stack examples in mini-batch
    ...
    def createTrainingLoader(self, 
        ...
        """Creating training data loader for EEGSEP system.
        For a single training simulation, the total time-series is sub-chunked into
        smaller blocks for training.
...
class AutoDataHandler():
    ...
    def load_data_handler(cls, model_name: str, **kwargs) -> EmbeddingDataHandler:
        ...
        Currently supports: "lorenz", "cylinder", "grayscott"; "EEGSEP" to be added soon
